...
Uwaga 4: Jesli podczas dzia≥ania skryptu pojawia siÍ b≥πd:
            "ORA-12560: TNS- b≥πd adaptera protoko≥u",
	 podaj úcieøkÍ dostepu do katalogu bin serwera Oracle jako drugi parametr podczas wywo≥ania skryptu:
		quickrman.bat uøytkownik/has≥o@instancja úcieøka
	 ,gdzie úcieøka - lokalizacja katalogu bin serwera bazy danych w podwÛjnym cudzys≥owiu,
...
UØYCIE: w celu uøycia skryptu naleøy wykonaÊ komendÍ:
	...
	[úcieøka] - opcjonalny parametr - lokalizacja katalogu bin serwera bazy danych
		    ze znakiem "\" na koÒcu, np. "c:\app\Administrator\product\11.2.0\dbhome_1\BIN\".
                    åcieøka powinna byÊ podana w podwÛjnym cudzys≥owiu (").
...
2. Sprawdzenie/Konfiguracja parametrÛw trybu Archivelog.
   W tym menu uøytkownik ma moøliwoúÊ sprawdzenia lub zmiany nastÍpujπcych parametrÛw:
     ...
     e) LOG_ARCHIVE_FORMAT - okreúla szablon nazw uøywany dla zarchiwizowanych dziennikÛw powtÛrzeÒ, gdzie %S - numer sekwencji, %R - resetlogs ID, %T - numer wπtku. Ten parametr moøe byÊ zmieniony jedynie w pliku SPFILE.
...
4. Sprawdzenie parametrÛw programu RMAN.

B. ZARZ•DZANIE KOPIAMI BEZPIECZE—STWA.

5. Sprawdzanie zajÍtoúci obszaru FRA.
...
6. Sprawdzenie listy plikÛw przestarza≥ych (do usuniÍcia) we FRA.
...
7. Usuwanie przestarza≥ych plikÛw z FRA
